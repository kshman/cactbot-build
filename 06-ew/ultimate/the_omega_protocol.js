const getMemberByName = (data, name) => data.members?.find((e) => e.n === name);
const getMemberRole = (data, name) => {
  const m = getMemberByName(data, name);
  return m ? m.r : data.party.member(name);
};
const testSynergyMarkerMove = (my, ot) => {
  if (my.sm < 5)
    return my.sm > ot.sm;
  return my.sm < ot.sm;
};
...
Options.Triggers.push({
  ...
  triggers: [
    ...
    },
    {
      ...
      delaySeconds: 0.5,
      durationSeconds: (data) => data.panked ? 5 : 38,
      suppressSeconds: 1,
      infoText: (data, _matches, output) => {
