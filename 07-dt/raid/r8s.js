  ...
  triggers: [
    ...
    },
    {
      ...
      infoText: (data, _matches, output) => {
        ...
        const intersToCard = (dirNum1, dirNum2) => {
        ...
        if (data.moonbeams.length === 2) {
          if (safe1.length === 2) {
            if (safe1[0] === undefined || safe1[1] === undefined) {
              console.error(`R8S Beckon Moonlight Quadrants: Early safeQuad missing.`);
              return;
            }
        ...
        if (safe1.length === 2 && safe2.length === 2) {
          if (safe1[1] === undefined || safe2[1] === undefined) {
            console.error(`R8S Beckon Moonlight Quadrants: Second safeQuads missing.`);
            return;
          }
        ...
        if (safe1.length === 2) {
          if (safe1[1] === undefined) {
            console.error(`R8S Beckon Moonlight Quadrants: First safeQuad missing.`);
            return;
          }
          const dirNum = intersToCard(safe1[0], safe1[1]);
          const half = output[AutumnDir.markFromNum(dirNum ?? -1)]();
        ...
        if (safe2.length === 2) {
          if (safe2[1] === undefined) {
            console.error(`R8S Beckon Moonlight Quadrants: Second safeQuad missing.`);
            return;
          }
